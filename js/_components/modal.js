...
function modalOpen(event){
  ...
  // enable all youtube videos with class, so they can be played
  $('.js-video-toggle').each(function() {
    var modalVideoSrc = $(this).data('src');
    $(this).attr('src', modalVideoSrc);
  });
...
function modalClose(event){
  ...
  // disable all youtube videos with class, so they stop playing when modal closed
  $('.js-video-toggle').each(function() {
    $(this).attr('src', '');
  });

  // reset scroll position - still shows up
...
// control youtube videos on modal
